// // components/gallery/MediaModal.tsx - UPDATED
// 'use client';
...
// export default function MediaModal({ item, onClose, onNext, onPrev }: MediaModalProps) {
//   // Keyboard navigation
//   useEffect(() => {
...
//   const isCloudinaryUrl = item.url.includes('cloudinary.com');
...
//           <button
//             onClick={onPrev}
//             className="absolute left-4 top-1/2 -translate-y-1/2 text-white hover:text-gray-300 z-20 transition"
//           >
...
//           <button
//             onClick={onNext}
//             className="absolute right-4 top-1/2 -translate-y-1/2 text-white hover:text-gray-300 z-20 transition"
//           >
...
//             {/* Use Next.js Image for local, regular img for Cloudinary */}
//             {isLocalUrl ? (
//               <Image
//                 src={item.url}
//                 alt={item.title}
...
//               )}
//               {item.location && (
//                 <p className="flex items-center gap-2 text-white/80 text-base md:text-lg">
...
interface MediaModalProps {
  ...
  onNext?: () => void;
  onPrev?: () => void;
  onDelete?: (item: GalleryItem) => Promise<void>; // ✅ ADD THIS LINE
}

export default function MediaModal({ item, onClose, onNext, onPrev, onDelete }: MediaModalProps) { // ✅ ADD onDelete HERE
  // Keyboard navigation
  useEffect(() => {
    const handleKey = (e: KeyboardEvent) => {
      if (e.key === 'Escape') onClose();
